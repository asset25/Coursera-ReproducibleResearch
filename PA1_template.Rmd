...
The mean number of steps taken per day is **`r format(meanstepsperday, scientific=FALSE)`**. The median number of steps taken per day is **`r format(medianstepsperday, scientific=FALSE)`**. 
...
On average across all days in the dataset, the 5-minute interval that contains the most steps is the **`r maxmeansteps`** interval.
...
The total number of rows with missing data in the dataset is **`r totalrowswithNA`**.
...
The mean number of steps taken per day is now **`r format(filledmeanstepsperday, scientific=FALSE)`**. The median number of steps taken per day is now **`r format(filledmedianstepsperday, scientific=FALSE)`**. Having filled in the missing values with the mean steps per interval has resulted in a higher value for the mean number of steps per day (`r format(meanstepsperday, scientific=FALSE)` vs. `r format(filledmeanstepsperday, scientific=FALSE)`). Interestingly, the median for the dataset is now equal to the mean.
